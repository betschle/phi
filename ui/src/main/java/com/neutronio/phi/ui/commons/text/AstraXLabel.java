...
/**
 * Mostly a copy of {@link Label}, just with a reactive background
 */
public class AstraXLabel extends Widget{
    ...
    public AstraXLabel(ComponentFactory componentFactory, String text, Label.LabelStyle labelStyle, String backgroundStyle) {
        this(componentFactory, text, labelStyle,
                backgroundStyle != null ? componentFactory.getSkin().get(backgroundStyle, Background.BackgroundStyle.class) : null );
    }
    ...
    public AstraXLabel(ComponentFactory componentFactory, String text, String labelStyle, String backgroundStyle) {
        this(componentFactory, text,
                labelStyle != null ? componentFactory.getSkin().get(labelStyle, Label.LabelStyle.class) :
                                     componentFactory.getSkin().get("default", Label.LabelStyle.class),
                backgroundStyle != null ? componentFactory.getSkin().get(backgroundStyle, Background.BackgroundStyle.class) :
                                          null
        );
    }
    ...
    public AstraXLabel(ComponentFactory componentFactory, String text, Label.LabelStyle labelStyle, Background.BackgroundStyle backgroundStyle) {
        ...
        if( backgroundStyle != null ) this.setBackgroundStyle(backgroundStyle);
    ...
    /**
     * Changes the background style for this tab. Duplicate code with AbstractPanel
...
//        this.pad(backgroundStyle.padding);
    }
    ...
    private void computePrefSize () {
        ...
        GlyphLayout prefSizeLayout = this.prefSizeLayout;
        ...
        if (wrap || text.indexOf("\n") != -1) {
            ...
            if ((labelAlign & Align.left) == 0) {
                if ((labelAlign & Align.right) != 0)
                    x += width - textWidth;
//                    x += width - (textWidth - this.background.getStyle().padding);
                else
                    x += (width - textWidth) / 2f;
//                    x += (width - (textWidth - this.background.getStyle().padding)) / 2;
            }
        } else {
            textWidth = width;
    ...
    public void draw (Batch batch, float parentAlpha) {
        ...
        if( this.background != null) {
            this.background.setColor( this.background.getCurrentColor().r,
                                      ...
                                      this.background.getCurrentColor().a * parentAlpha * tempColor.a);
            this.background.setSize(getWidth(), getHeight());
//            this.background.setSize(getPrefWidth(), getPrefHeight()); // TODO this does not act properly, pos will be off in table layouts
...
//        }
    ...
    public float getPrefWidth () {
        ...
        if (this.background != null) {
            ...
            width += this.background.getStyle().padding; // this is probably not 100% correct but works
        }
        return width;
    }
    ...
    public float getPrefHeight () {
        ...
        if (this.background != null) {
            ...
            height += this.background.getStyle().padding;  // this is probably not 100% correct but works
        }
        return height;
    }
